...
/****** DMIF ******/
Entry*
Fib::findExactMatch_dmif(const Name& prefix, uint32_t forwarderId) {
	try {
		LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.start");
//		LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.prefix", prefix.toUri());
//		LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.forwarderId", forwarderId);

		fib::Entry* fibEntry;
		name_tree::Entry* nte = m_nameTree.findExactMatch_dmif(prefix, forwarderId);
		...
		if (fibEntry != nullptr)
			LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.found");
		else
			LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.not-found");

		LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.completed");

		return fibEntry;

	} catch (std::exception& ex) {
		LogManager::AddLogWithNodeId("fib.cpp->findExactMatch_dmif.exception", ex.what());
		return nullptr;
	}
}
/****** DMIF ******/
