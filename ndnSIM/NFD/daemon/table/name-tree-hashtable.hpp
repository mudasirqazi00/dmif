...
public:
  ...
  const Node*
  find_dmif(const Name& name, size_t prefixLen, uint32_t forwarderId) const;
...
private:
  ...
  /****** DMIF *******/
  std::pair<const Node*, bool>
  findOrInsert_dmif(const Name& name, size_t prefixLen, HashValue h, bool allowInsert, uint32_t forwarderId);
  /****** DMIF *******/
