...
{
	LogManager::AddLogWithNodeId("link-service.cpp->sendNack.start");
  BOOST_ASSERT(m_transport != nullptr);
  NFD_LOG_FACE_TRACE(__func__);
  ...
  afterSendNack(nack);
  LogManager::AddLogWithNodeId("link-service.cpp->sendNack.completed");
}
...
LinkService::receiveData(const Data& data)
{
	...
	LogManager::AddLogWithNodeId("link-service.cpp->receiveData.forwarderId", data.getForwarderId());
  NFD_LOG_FACE_TRACE(__func__);
