...
public:
	Data&
	setResidualEnergy(const uint32_t val);

	uint32_t
	getResidualEnergy() const;

	Data&
	setInitialHop(const uint32_t val);

	uint32_t
	getInitialHop() const;

	Data&
	setForwarderId(const uint32_t val);

	uint32_t
	getForwarderId() const;

	Data&
	setIdsList(const std::vector<int> ids);
	...
	Data&
	addIdInIdsList(const int id);

	template<encoding::Tag TAG>
	size_t
	wireEncodeIdsList(EncodingImpl<TAG>& encoder) const;

	std::vector<int>
	wireDecodeIdsList(const Block& wire);
/***** DMIF ******/
...
private:
  ...
  /***** DMIF ******/
  uint32_t m_residualEnergy;
  uint32_t m_initialHop;
  uint32_t m_forwarderId;
